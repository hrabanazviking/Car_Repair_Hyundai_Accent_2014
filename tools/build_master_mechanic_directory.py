...
def render_record(record):
    ...
    lines.append("**Directory source files:** " + ", ".join(f"`{s}`" for s in srcs) + "  ")
...
def main():
    ...
    out.append("")
    out.append("> **Canonical combined directory.** This file is generated from the original nationwide directory plus every expansion file currently in this repository. It groups all shop records by state, merges repeated shop records when the same source URL or normalized shop/location heading appears more than once, and preserves the research notes, watchlists, traveler guidance and quality-control material from the source documents in an appendix.")
    out.append(">")
    out.append("> **Research date represented by the current source set:** 2026-09-13")
    out.append(f"> **Source directory files combined:** {len(sources)}")
    ...
    out.append("- `Directory source files` shows where each entry came from, preserving the audit trail.")
    ...
    out.append("")
    out.append("This appendix preserves non-state material from every source directory file so the master document does not lose research methodology, warnings, review watchlists, traveler scripts, strongest-find summaries, limitations or future-expansion guidance.")
    out.append("")
    ...
    OUTPUT.write_text("\n".join(out).rstrip() + "\n", encoding="utf-8")
    print(f"Wrote {OUTPUT.name}: {len(all_entries)} raw entries -> {unique_count} merged records from {len(sources)} source files")
